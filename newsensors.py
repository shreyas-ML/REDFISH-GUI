...
def print_sensors( sensor_list ):
    ...
    for chassis in sensor_list:
        ...
        for reading in chassis["Readings"]:
            ...
            #print ( reading_pr["Name"])

        #print(" ")



def oem_sens(context):
    ...
    manager_oem = context.get(manager.dict["Links"]["Oem"]["Sensors"]["@odata.id"])
    #manager_oem = context.get("/redfish/v1/Managers/Self/OemSensors")

    #sensor_line_format = "  {:25s} | {:10s} | {:8s} | {:8s} | {:8s} | {:8s} | {:8s} | {:8s} | {:8s} "
    na ="N/A"
    for sen in manager_oem.dict["Sensors"]:
        print(sensor_line_format.format(sen,na,na,na,na,na,na,na,na,na) )
